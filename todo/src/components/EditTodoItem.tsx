...
export const EditTodoItem: React.FC<Props> = ({
  ...
  <tr
    key={todoItem.todoItemIdentifier}
    className={todoItem.todoItemCompleted ? "bg-gray-200" : ""}
  >
    <td className="border border-gray-300 px-4 py-2 text-center">
      <label className="relative inline-flex cursor-pointer items-center">
        <input
          ...
          onChange={() => toggleCompletion(todoItem.todoItemIdentifier)}
    ...
    <td className="border border-gray-300 px-4 py-2">
      <input
        type="text"
        value={todoItem.todoItemTitle}
        disabled={todoItem.todoItemCompleted}
        onChange={(e) =>
          handleEdit(
            todoItem.todoItemIdentifier,
            "todoItemTitle",
            e.target.value,
          )
        }
        onBlur={() => saveChanges(todoItem.todoItemIdentifier)}
    ...
    <td className="border border-gray-300 px-4 py-2">
      <input
        type="text"
        value={todoItem.todoItemDescription || ""}
        disabled={todoItem.todoItemCompleted}
        onChange={(e) =>
          handleEdit(
            todoItem.todoItemIdentifier,
    ...
    <td className="text-center">
      <button
        onClick={() => handleDelete(todoItem.todoItemIdentifier)}
